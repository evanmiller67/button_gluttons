class Player < ActiveRecord::Base
  ...
  class << self
    ...
    def winners;    where(:is_boss => false).registered.order("wins desc").order("losses asc").limit(5); end
    def bosses;     where(:is_boss => true).registered.order("wins desc").order("losses asc").limit(5); end
